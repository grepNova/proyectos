...
public class DespachoHome {
	...
	public void remove(Despacho persistentInstance) {
		log.debug("removing Despacho instance");
		try {
	...
	public Despacho merge(Despacho detachedInstance) {
		...
		try {
			Despacho result = entityManager.merge(detachedInstance);
	...
	public Despacho findById(int id) {
		...
		try {
			Despacho instance = entityManager.find(Despacho.class, id);
